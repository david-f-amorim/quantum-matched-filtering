...
k_plot = False # Fig 4a
ck_plot = False # Fig 4b
rho_plot = False # Fig 4c

CSC_plot = True # Fig 3
CSC_phase_plot =False # complex version of Fig 3 (not in report)
...
if k_plot: # generate fidelities for Figure 4a
...
if ck_plot: # generate fidelities for Figure 4b [note: c(k) has been replaced with q(k) in the report]
...
if rho_plot: # generate fidelities for Figure 4c
...
titlesize=32
ticksize=22
figsize=(9,8) # 10,6 (9,8) for rescaled
...
if CSC_plot:
    ...
    fig.set_facecolor('white') 
    ax = plt.gca()
    plt.xlabel(r'no. ancillae in $| 0 \rangle $',fontsize=1.3*fontsize)
    plt.ylabel(r'Amplitude',fontsize=1.3*fontsize)
    arrowed_spines(fig, ax)
...
# plot generalisation of Figure 3 showing complex phases (not used in report)
if CSC_phase_plot:
